...
export function occupancyReason(
...
export function bookingSourceLabel(source: BookingSource | string): string {
  switch (source) {
    case "player":
      return "Jugador";
    case "owner":
...
export function adminOccupancyKindLabel(kind: AdminOccupancyKind): string {
  switch (kind) {
    case "match":
      return "Hueco";
    case "booking_hold":
...
/** Combina fila de ocupación del día + fila booking enriquecida (status/source). */
export function toAdminOccupancyKind(input: {
  blockKind: OccupancyBlockKind;
  bookingStatus?: string | null;
  bookingSource?: string | null;
}): AdminOccupancyKind {
  if (input.blockKind === "match") return "match";
  if (input.bookingSource === "block") return "block";
