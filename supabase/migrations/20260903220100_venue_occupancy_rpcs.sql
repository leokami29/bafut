-- RPCs de ocupación y lado B + update_match (cupos A; no borrar B).
...
create or replace function public.open_match_side_b(
  p_match_id uuid,
...
begin
  ...
  if p_open_count is null or p_open_count not in (1, 2) then
    raise exception 'El lado B admite 1 o 2 cupos.';
  end if;

  select host_id, status, starts_at, sport, away_opened_by, share_code
    into v_host, v_status, v_starts, v_sport, v_away, v_share
  from public.matches
  where id = p_match_id
  for update;
  ...
  if v_host is not distinct from v_uid then
    raise exception 'El lado B es el equipo en contra. Si faltan de tu grupo, uníte a los cupos.';
  end if;
  if v_away is not null then
    raise exception 'El lado B ya está abierto';
  end if;

  v_pos := coalesce(nullif(btrim(p_position), ''), 'any');
  if not private.sport_allows_position(v_sport, v_pos) then
    raise exception 'Esa posición no aplica para el deporte.';
  end if;
  if coalesce(p_level, 'any') not in ('any', 'low', 'mid', 'high') then
  ...
  for v_i in 1 .. p_open_count loop
    insert into public.match_slots (match_id, position, level, side)
    values (p_match_id, v_pos, coalesce(p_level, 'any'), 'b');
  end loop;
...
comment on function public.open_match_side_b(uuid, integer, text, text) is
  'SECURITY DEFINER: abre lado B (rival) una vez. No ampliar a host de A ni a un segundo matches.';

revoke all on function public.open_match_side_b(uuid, integer, text, text) from public, anon;
grant execute on function public.open_match_side_b(uuid, integer, text, text) to authenticated;
...
begin
  ...
  for v_i in 0 .. v_n - 1 loop
    ...
    if v_slot_id is null then
      insert into public.match_slots (match_id, position, level, side)
      values (p_match_id, v_pos, v_level, 'a');
    else
      select exists (
        select 1 from public.slot_claims c
